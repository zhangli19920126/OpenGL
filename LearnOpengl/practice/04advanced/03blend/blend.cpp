...
// ps: The rendering order of transparent objects is particularly important!!!
//1. Rendering opaque objects first
//2. Sort transparent objects by distance from the camera and render them(render those farther away first)
//3. Even though, the transparent rendering may still be incorrect
...
float cubePos[3] = { -1.0f, 0.0f, -1.0f };
float transparentPos[3] = { 0.0f, 0.0f, 0.0f };
...
int main()
{
    ...
    Shader shader("practice/04advanced/03blend/blend.vs", "practice/04advanced/03blend/blend.fs");
    ...
    unsigned int transparentTexture = imgLoader.loadTexture("resources/textures/window.png");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3(cubePos[0] + 3.0f, cubePos[1], cubePos[2]));
        shader.setMat4("model", model);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        ...
        {
            model = glm::mat4(1.0f);
            model = glm::translate(model, it->second + glm::vec3(transparentPos[0], transparentPos[1], transparentPos[2]));
            shader.setMat4("model", model);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
        ...
        ImGui::SliderFloat3("transparent pos", transparentPos, -6.0f, 6.0f);
